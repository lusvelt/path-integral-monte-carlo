...
def CLI() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--from-history",
        action="store_true",
        help="Boolean flag to use --from-history flag with `conda env export`.",
    )
    parser.add_argument(
        "-nob",
        "--no-builds",
        action="store_true",
        help="Apply --no-builds flag to `conda env export`.",
    )
    parser.add_argument(
        "-v",
        "--use-versions",
        action="store_true",
        help="Boolean flag to include package version numbers when using --from-history flag",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Boolean flag to indicate if output to file should also be printed to terminal",
    )

    parser.add_argument(
        "--include-prefix",
        action="store_true",
        help="Boolean flag to include the `prefix` line that conda appends to an environment file",
    )
    parser.add_argument(
        "-n",
        "--env-name",
        type=str,
        default=None,
        help="Give a name to the conda environment. If not provided, use current environment's name",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        default=None,
        help="Specify an output file to save with environment data. If not provided, it will be printed to the terminal",
    )
